...
public class TenantDataSourcePortFixService {
	 ...
	 * completely disable it if you've hand tuned your tenants table for an advanced
	 * configuration and use e.g. different databases maybe on different hosts for
	 * different tenants.
	 */
	...
	public void fixUpTenantsSchemaServerPort() {
		if (!enabled) {
			logger.info(
					"No schema_server_port UPDATE made to tenant_server_connections table of the mifosplatform-tenants schema, because "
							+ ENABLED + " = false");
			return;
		}
		if (dsp == null) {
			// we don't have any generic mechanism to know the DB port, given just a tenant
			// DataSource
			logger.debug(
		...
		if (r == 0)
			logger.warn(
					"UPDATE tenant_server_connections SET ... did not update ANY rows - something is probably wrong");
		else
